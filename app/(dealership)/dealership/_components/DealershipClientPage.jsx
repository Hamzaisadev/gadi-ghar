...
import {
  ...
  getDealershipById,
  getDealershipCarsById,
} from "@/app/actions/dealership";
import { formatPriceRange } from "@/components/utils/FormatCurrency";
import TestDrivesTabContent from "./TestDrivesTabContent";

export default function DealershipClientPage({
  initialDealership,
  initialCars,
  dealershipId,
  isAdminView = false,
  ...
  // Fetch data when dealershipId is provided (for admin view)
  useEffect(() => {
    if (dealershipId && isAdminView) {
      fetchDealershipData();
    }
  }, [dealershipId, isAdminView]);
  ...
  const fetchDealershipData = async () => {
    ...
    setIsLoading(true);
    try {
      const [dealershipResult, carsResult] = await Promise.all([
        getDealershipById(dealershipId),
        getDealershipCarsById(dealershipId),
      ]);

      if (dealershipResult.success) {
        setDealership(dealershipResult.data);
        setEditForm(dealershipResult.data);
      }
  ...
  const handleEditSubmit = async (e) => {
    ...
    try {
      ...
      if (!result.success) {
        throw new Error(
          result.error || "Failed to update dealership information"
        );
      }
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              Rs. {dealership.totalSales?.toLocaleString() || "0"}
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div>
                ...
                <Select
                  ...
                  onValueChange={(value) =>
                    setNewCar({ ...newCar, bodyType: value })
                  }
                >
                  <SelectTrigger>
