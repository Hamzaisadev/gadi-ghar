...
import { getDealershipCars, deleteDealershipCar, updateDealershipCar } from "@/app/actions/dealership-cars";
...
const DealerCarList = () => {
  ...
  // Fetch cars specific to this dealership
  const {
    loading: loadingCars,
    fn: fetchDealerCars,
    data: carsData,
  } = useFetch(getDealershipCars);

  const {
    loading: deletingCar,
    fn: deleteCarFn,
  } = useFetch(deleteDealershipCar);

  const {
    loading: updatingCar,
    fn: updateCarStatusFn,
  } = useFetch(updateDealershipCar);

  // Fetch dealership and cars on mount
  useEffect(() => {
    fetchDealership();
    fetchDealerCars(""); // Initial fetch with empty search
  }, []);

  // Set dealership when data loads
  useEffect(() => {
  ...
  // Fetch cars when search changes
  useEffect(() => {
  ...
  return (
    <>
        ...
        <div className="flex justify-between items-center">
          <div>
            ...
            <p className="text-car-gray text-lg">
              Manage your dealership inventory ({dealership.name})
            </p>
          </div>
          <Button
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 p-6">
        ...
        {localCars.length === 0 && !loadingCars && (
