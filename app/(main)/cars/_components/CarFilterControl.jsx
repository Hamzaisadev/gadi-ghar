...
const CarFilterControls = ({
  ...
  // Format price in Lakhs or Crores (starting from 10 Lakhs)
  const formatPriceInLakhs = (price) => {
    if (price >= 10000000) { // 1 Crore = 100 Lakhs
      return (price / 10000000).toFixed(2) + ' Crore';
    } else if (price >= 1000000) { // 10 Lakhs = 1,000,000
      return (price / 100000).toFixed(0) + ' Lakhs';
    } else {
      return (price / 100000).toFixed(0) + ' Lakhs'; // Show in Lakhs even below 10 Lakhs
    }
  };
      ...
      <div className="space-y-4">
        ...
        <div className="px-2">
          <Slider
            ...
            max={Number(filters.priceRange.max) || 100000}
            step={1000}
            value={priceRange}
        ...
        <div className="flex items-center justify-between">
          <div className="font-medium text-sm">
            PKR {formatPriceInLakhs(priceRange[0])}
          </div>
          <div className="font-medium text-sm">
            PKR {formatPriceInLakhs(priceRange[1])}
          </div>
        </div>
      </div>
