...
const OptimizedImage = forwardRef(
  (
    ...
    // Ensure we never pass an empty string or undefined to next/image
    const initialSrc = typeof src === 'string' && src.trim().length > 0 ? src : fallbackSrc;
    const [currentSrc, setCurrentSrc] = useState(initialSrc);

    // Reset state when src prop changes
    useEffect(() => {
      const newSrc = typeof src === 'string' && src.trim().length > 0 ? src : fallbackSrc;
